...
describe Drink do
  describe '#initialize' do
    context 'when type is specified and ounces is default' do
      ...
      # beverage type.
      subject(:default_drink) { described_class.new('juice')  }
      # remove the 'x' before running this test
      it 'is your choice of beverage' do
        expect(default_drink.type).to eq 'juice'
      end
  ...
  describe '#full?' do
    context 'when drink has 16 ounces or more' do
      # Create an explicit subject, using 'described_class' and your choice of
      # beverage type.
      subject(:test_drink) { described_class.new('milk', 18) }
      # remove the 'x' before running this test
      it 'is full' do
        expect(test_drink).to be_full
      end
    end
    ...
    context 'when drink has less than 16 ounces' do
      ...
      subject(:emtpy_drink) { described_class.new('milk', 10) }
      # remove the 'x' before running this test
      it 'is not full' do
        expect(emtpy_drink).not_to be_full
      end
    end
  end
end
