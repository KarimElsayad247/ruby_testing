...
describe FindNumber do
  ...
  describe '#game_over?' do
    context 'when guess and random_number are equal' do
      ...
      subject(:done_game) { described_class.new(0, 9, random_number, 5) }
      ...
      it 'is game over' do
        ...
        expect(done_game).to be_game_over
      end
    end
    ...
    context 'when guess and random_number are not equal' do
      subject(:not_done_game) { described_class.new(0, 9, random_number, 4) }
      let(:random_number) { double('random_number') }

      it 'is not game over' do
        allow(random_number).to receive(:value).and_return(7)
        expect(not_done_game).not_to be_game_over
      end
    end
  end
  ...
  describe '#update_range' do
    ...
    context 'when the guess is 7, min=5, and max=8' do
      ...
      subject(:guess_game) { described_class.new(5, 8, random_number, 7) }
