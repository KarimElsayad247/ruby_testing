...
describe Cat do
  ...
  subject(:boo) { described_class.new(name: "Boo", breed: "Tabby", color: "Silver")}
  # Write a test using the second shared_example to test that cat responds to
  # talk ('meow').
  context 'when Cat is child of Pet class' do
    include_examples 'shared method name'
  end

  # remove the 'x' before running this test
  it 'is not hungry' do
    expect(boo).not_to be_hungry
  end

  # remove the 'x' before running this test
  it 'is hiding' do
    expect(boo).to be_hiding
  end
end
